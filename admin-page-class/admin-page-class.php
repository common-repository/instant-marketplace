...
if ( ! class_exists( 'BF_Admin_Page_Class') ) :
  ...
  class BF_Admin_Page_Class {
    ...
    public function wp_enqueue_color_picker( $hook_suffix ) {
      ...
      wp_enqueue_script( 'wp-color-picker-script-handle', plugins_url('js/Admin_Page_Class.js', __FILE__ ), array( 'wp-color-picker' ), false, true );
    }
    ...
    public function DisplayPage() {
      $p1xtr_plugin_user = get_option($this->args['plugin_name'] . '_user');
      $p1xtr_plugin_is_guest_user = get_option($this->args['plugin_name'] . '_is_guest_user');
      do_action($this->args['plugin_name'] . '_admin_page_class_before_page');
      echo '<div class="p1xtr-wrapper wrap">'."\n";
      echo '<h2>'.apply_filters('admin_page_class_h2',$this->args['page_title']).'</h2>';
      if (empty($p1xtr_plugin_user) || $p1xtr_plugin_is_guest_user == true){
        do_action($this->args['plugin_name'] . '_admin_page_class_display_register_page');
      }
      echo '<div class="admin-page-container">'."\n";
      echo '<form method="post" style="display:'.((empty($p1xtr_plugin_user))? 'none' : 'block' ) .';" name="'.apply_filters('apc_form_name', 'admin_page_class',$this).'" class="'.apply_filters('apc_form_class', 'admin_page_class',$this).'" id="'.apply_filters('apc_form_id', 'admin_page_class',$this).'" action="" enctype="multipart/form-data">
        '."\n".'<div class="header_wrap">'."\n".
          ((isset($this->args['page_header_text']))? '<h4 class="page-notice">'.$this->args['page_header_text'].'</h4>' : '').
          '<div style="float:left">'."\n";
      echo apply_filters('admin_page_class_before_title','');
      echo "\n".
          '</div>'."\n".
          '<div style="float:right;margin:32px 0 0 0">'."\n".
          ((empty($p1xtr_plugin_user) || $p1xtr_plugin_is_guest_user == true)? '<input type="button" class="register-btn btn btn-info" value="Register"></input>' : '').
    ...
    public function load_scripts_styles() {
      ...
      foreach (array('upload','color','date','time','code','select','editor','plupload') as $type) {
        if(function_exists('check_field_' . $type )){
...
.at-repater-block{background-color: #FFFFFF;border: 1px solid;margin: 2px;}
    ...
    public function show_field_cond( $field, $meta ) {
      $p1xtr_plugin_options = get_option($this->args['plugin_name'] .'_options');
      $this->show_field_begin($field, $meta);
      $checked = false;
      if($p1xtr_plugin_options[$field['id']] && !is_array($meta)){
        $meta = array(
            "enabled" => "on",
        );
      }
